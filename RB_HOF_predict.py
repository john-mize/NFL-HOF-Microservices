...
# import file
# statsDF = pd.read_csv('/home/john/Documents/nfl_hof_app/rbstats.csv')
statsDF = pd.read_csv('rbstats.csv')

# drop non RBs from dataframe
statsDF = statsDF.drop(statsDF[statsDF.Pos != "RB"].index)
...
Xtrain = statsDFtrain.drop(['Rank', 'Player', 'First Year', 'Pos', 'HOF_Eligible', 'HOF'], axis=1)
ytrain = statsDFtrain['HOF']

Xtest = statsDFtest.drop(['Rank', 'Player', 'First Year', 'Pos', 'HOF_Eligible', 'HOF'], axis=1)
...
HOF_predict["Predicted HOF Running Backs"] = HOFplayers
...
# also recombine the statsDFtrain, statsDFtest, and the predictions into an object that will be accessed later in the app.py file
# should have HOF: yes/no, predicted HOF: yes/no